...
from src.core.exceptions import Error, TooManyCogsError, TooManyToonsError
from src.core.toon import Toon
from src.factories.utils import create_battle_entity
from src.gags.gag import get_gag_track_name

from .attack import Attack
from .attack_globals import GROUP, MULTIPLIER
from .attack_processor import AttackProcessor
from .battle_entity import BattleEntity


# TODO class RewardTracker (Iterator design pattern? over Battle TimeSteps), remove `calculate_rewards` from Battle
class Battle:
    ...
    def __init__(
        self, building_floor: int = MULTIPLIER.FLOOR1, is_invasion: bool = False
    ):
        self.building_floor = building_floor
        self.is_invasion = is_invasion
    ...
    @property
    def cogs(self) -> List[BattleEntity]:
        return self._cogs
    ...
    def add_cog(self, new_cog: Cog) -> None:
        ...
        if len(self._cogs) >= 4:
            print(
                f"    [!] ERROR : Too many Cogs battling, can't add Cog " f"{new_cog}"
            )
            raise TooManyCogsError(new_cog)
        self._cogs.append(
            create_battle_entity(battle_id=self._cog_battle_id, entity=new_cog)
        )
    ...
    def add_toon(self, new_toon: Toon) -> None:
        ...
        if len(self._toons) >= 4:
            print(
                f"    [!] ERROR : Too many Toons battling, can't add Toon "
                f"{new_toon}"
            )
            raise TooManyToonsError(new_toon)
        battle_toon = create_battle_entity(
            battle_id=self._toon_battle_id, entity=new_toon
        )
        self.attach(btoon=battle_toon)
        self._toons.append(battle_toon)
        self._toon_battle_id += 1

    def attach(self, btoon: BattleEntity):
    ...
    def remove_battle_cog(self, bcog: BattleEntity):
        self._cogs.remove(bcog)

    def remove_battle_toon(self, btoon: BattleEntity):
...
class BattleContext:
    def __init__(
        self,
        state: BattleState,
        cogs: list[BattleEntity],
        toons: list[BattleEntity],
    ) -> None:
    ...
    def transition_to(self, new_state: BattleState):
        print(
            f"    [+] `transition_to()` transition : {self.state} -> {new_state}"
        )  # noqa
        self._completed_states.append(self.state)
        self.state = new_state
        print(
            f"        [-] `transition_to()` completed states : {[str(state) for state in self._completed_states]}"
        )  # noqa
        self.state.context = self
...
class ToonAttackState(AttackState):
    ...
    def __init__(self):
        self.attacks = (
            []
        )  # [(BattleEntity, BattleEntity, Gag, atk_hit_or_miss: int [0|1])]
        # Keep track of which Cogs are being targeted and by how many Toons
        self.overdefeat_cogs = {}
        self.attack_processor = AttackProcessor()
    ...
    def choose_attacks(self):
        ...
        for toon in self.context.toons:
            ...
            """
            alive_cogs = [cog for cog in self.context.cogs if not cog.is_defeated]
            action, targets = toon.choose_action(targets=alive_cogs)

            if not isinstance(action, Attack):
                print(f"            [-] BattleEntity {toon} passes/stalls")
                continue

            print(f"            [-] BattleEntity {toon} targets BattleEntity {targets}")
            # #44, Group attacks by Gag.track
            if action.track not in potential_attacks:
                potential_attacks[action.track] = []

            # Track which Cogs are being targeted and by how many Toons
            for target_cog in targets:
                if action.track not in self.overdefeat_cogs[target_cog]:
                    self.overdefeat_cogs[target_cog][action.track] = 0
                self.overdefeat_cogs[target_cog][action.track] += 1

            potential_attack = (toon, targets, action)
            potential_attacks[action.track].append(potential_attack)

        return potential_attacks

    def handle_attacks(self):
        potential_attacks = self.choose_attacks()
        ...
        for gag_track in sorted(potential_attacks):
            ...
            for toon, targets, action in potential_attacks[gag_track]:
                ...
                for cog in targets:
                    is_overdefeat = self.overdefeat_cogs[cog][action.track] > 1  # noqa
                    # Cache cog.trap.. if BattleEntity uses Lure and the Trap activates
                    cog_is_trapped = cog.is_trapped
                    if cog_is_trapped:
                        trap_toon, trap_gag = cog.trap

                    attack_hit = attack_proccessor.do_attack(
                        target=cog, action=action, overdefeat=is_overdefeat
                    )
                    # Attack doesn't miss and Gag is eligible for reward
                    if attack_hit:
                        # Activate the Trap Gag if the BattleEntity lures a trapped BattleEntity
                        if (
                            cog.is_lured and cog_is_trapped
                        ):  # Potential bug if trap is multi-target
                            self.attacks.append((trap_toon, cog, trap_gag, attack_hit))

                self.attacks.append((toon, targets, action, attack_hit))
...
class CogAttackState(AttackState):
    ...
    def handle_attacks(self):
        ...
        for cog in self.context.cogs:
            cog_atk = (
                cog.manual_atk if cog.manual_atk is not None else cog.choose_action()
            )  # noqa

            viable_toons = [toon for toon in self.context.toons if not toon.is_defeated]
            if cog_atk.target == GROUP.MULTI:
                print(
                    f"        [+] {self} BattleEntity {cog} targets all Toons "
                    f"{viable_toons}"
                )
                for toon in viable_toons:
                    ...
                    if toon.is_defeated:
                        print(
                            f"            [-] BattleEntity {target_toon} is defeated"
                        )  # noqa
                ...
                print(
                    f"        [+] {self} BattleEntity {cog} targets BattleEntity "
                    f"{target_toon}"
                )
...
class LoseState(WinLoseState):
    ...
    def handle_win_lose(self):
        ...
        defeated_toons = [toon for toon in self.context._toons if toon.is_defeated]
